...
const SCRIPTS_STATUS_FILE = path.join(__dirname, 'service-logs/scripts_status.json');
const ACTIVITY_LOGS_FILE = path.join(__dirname, 'service-logs/activity_logs.json');
...
try { scripts = JSON.parse(fs.readFileSync(SCRIPTS_FILE, 'utf8')); } catch { scripts = []; }
try { latestLogs = JSON.parse(fs.readFileSync(LOGS_FILE, 'utf8')); } catch { latestLogs = {}; }
try { scriptsStatus = JSON.parse(fs.readFileSync(SCRIPTS_STATUS_FILE, 'utf8')); } catch { scriptsStatus = {}; }
try { donutStatCache = JSON.parse(fs.readFileSync(DONUT_STAT_FILE, 'utf8')); } catch { donutStatCache = {}; }
try { activityLogs = JSON.parse(fs.readFileSync(ACTIVITY_LOGS_FILE, 'utf8')); } catch { activityLogs = {}; }
...
function saveScriptsStatusToFile() {
  fs.writeFile(SCRIPTS_STATUS_FILE, JSON.stringify(scriptsStatus, null, 2), err => {
    if (err) console.error('Erreur sauvegarde status :', err);
  });
}
function saveDonutStatToFile() {
  fs.writeFile(DONUT_STAT_FILE, JSON.stringify(donutStatCache, null, 2), err => {
    if (err) console.error('Erreur sauvegarde donut stat :', err);
  });
}
function saveActivityLogs() {
  fs.writeFile(ACTIVITY_LOGS_FILE, JSON.stringify(activityLogs, null, 2), err => {
...
io.on('connection', socket => {
  ...
  socket.on('identify', data => {
    ...
    scriptsStatus[name] = { connected: true, updatedAt: new Date().toISOString() };
    saveScriptsStatusToFile();
    io.emit('status', { name, connected: true, updatedAt: scriptsStatus[name].updatedAt });
  });

  // Déconnexion d'un script
  socket.on('disconnect', () => {
    if (socket.scriptName) {
      scriptsStatus[socket.scriptName] = { connected: false, updatedAt: new Date().toISOString() };
      saveScriptsStatusToFile();
      io.emit('status', {
  ...
  socket.on('activity_log', data => {
    ...
    saveActivityLogs();
    io.emit('activity_log', { name: data.name, log: activityLogs[data.name][activityLogs[data.name].length - 1] });
  });
